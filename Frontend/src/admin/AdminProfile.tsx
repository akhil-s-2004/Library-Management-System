...
import { BookOpen, Users, AlertCircle, TrendingUp, Pencil, X } from 'lucide-react'
import { fetchAdminProfile, updateAdminProfile, fetchAnalytics } from '../services/adminService'
...
const AdminProfile = () => {
    ...
    useEffect(() => {
        fetchAdminProfile().then(setUser).catch(err => console.error('Error fetching admin profile:', err))
        fetchAnalytics().then(setAnalytics).catch(err => console.error('Error fetching analytics:', err))
    }, [])
    ...
    const displayStats = [
        { icon: <BookOpen size={20} className='text-[#570000]' />, label: 'Total Books', value: analytics?.total_books ?? '—' },
        { icon: <Users size={20} className='text-[#570000]' />, label: 'Total Members', value: analytics?.total_members ?? '—' },
        { icon: <TrendingUp size={20} className='text-[#570000]' />, label: 'Active Issues', value: analytics?.active_issues ?? '—' },
        { icon: <AlertCircle size={20} className='text-[#570000]' />, label: 'Outstanding Fines', value: analytics?.outstanding_fines != null ? `₹${Math.round(Number(analytics.outstanding_fines))}` : '—' },
    ]

    return (
        <div className='px-4 sm:px-8 lg:px-10 py-6 sm:py-8 flex flex-col gap-6 sm:gap-8 max-w-5xl mx-auto'>
            <h1 className='text-xl sm:text-2xl font-bold text-[#570000]'>Admin Profile</h1>
            ...
            <div className='bg-white border border-[#e8d4d4] rounded-3xl shadow-sm overflow-hidden'>
                ...
                <div className='px-4 sm:px-8 pb-6 sm:pb-8'>
                    ...
                    <div className='flex flex-col gap-1'>
                        ...
                        {user.membershipNumber && <p className='text-gray-500 text-sm'>ID: {user.membershipNumber}</p>}
                        <div className='flex gap-2 mt-2 flex-wrap'>
                            <span className='bg-[#f4e7e5] text-[#570000] text-xs font-semibold px-3 py-1 rounded-full border border-[#e8c8c8] uppercase'>{user.role}</span>
                            <span className='bg-[#f4e7e5] text-[#570000] text-xs font-semibold px-3 py-1 rounded-full border border-[#e8c8c8] uppercase'>{user.status}</span>
                        </div>
                    </div>
                </div>
            </div>
